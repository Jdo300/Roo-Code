...
- **`data`** (object, required): The actual `TaskCommand` object containing:
    - **`requestId`** (string, required): A unique identifier generated by the client for this specific command request. Used to correlate responses.
    - **`commandName`** (string, required): The name of the command to execute (e.g., "StartNewTask", "SendMessage").
    - **`data`** (any, required): Command-specific parameters payload. The structure varies depending on the `commandName`. See the table below.
