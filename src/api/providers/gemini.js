...
export class GeminiHandler extends BaseProvider {
	...
	async *createMessage(systemInstruction, messages, cacheKey) {
	...
	getModel() {
		let id = this.options.apiModelId ? this.options.apiModelId : geminiDefaultModelId
	...
	calculateCost({ info, inputTokens, outputTokens, cacheWriteTokens = 0, cacheReadTokens = 0 }) {
