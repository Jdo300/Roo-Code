...
export class WebSocketServer {
    ...
    constructor(provider: ClineProvider | null) {
        ...
        this.commandHandler = new CommandHandler(provider as ClineProvider); // provider can be null initially but commandHandler needs ClineProvider
        this.outputChannel = vscode.window.createOutputChannel("Roo-Code WebSocket");
    }

    public setProvider(provider: ClineProvider): void {
        this.provider = provider;
        this.commandHandler = new CommandHandler(provider);
    }
    ...
    public async start(): Promise<void> {
        ...
        try {
            ...
            this.outputChannel.appendLine(`Failed to start WebSocket server: ${error instanceof Error ? error.message : String(error)}`);
            vscode.window.showErrorMessage(`Failed to start WebSocket server: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    private handleMessage(message: ws.RawData, client: ws.WebSocket): void {
        try {
            const parsedMessage: WebSocketMessage = JSON.parse(message.toString());
            this.outputChannel.appendLine(`Received message: ${JSON.stringify(parsedMessage)}`);
            this.processMessage(parsedMessage, client);
        } catch (error) {
            this.outputChannel.appendLine(`WebSocket message parsing error: ${error instanceof Error ? error.message : String(error)}`);
            vscode.window.showErrorMessage(`WebSocket message parsing error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }

    private processMessage(message: WebSocketMessage, client: ws.WebSocket): void {
        try {
            if (!this.commandHandler) {
                throw new Error('CommandHandler not initialized');
            }
            this.commandHandler.processCommand(message, client);
        } catch (error) {
