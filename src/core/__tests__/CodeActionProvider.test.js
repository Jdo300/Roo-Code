...
describe("CodeActionProvider", () => {
	...
	describe("provideCodeActions", () => {
		it("should provide explain, improve, fix logic, and add to context actions by default", () => {
			...
			expect(actions[0].title).toBe(ACTION_NAMES.ADD_TO_CONTEXT)
			expect(actions[1].title).toBe(`${ACTION_NAMES.EXPLAIN} in New Task`)
			expect(actions[2].title).toBe(`${ACTION_NAMES.EXPLAIN} in Current Task`)
			expect(actions[3].title).toBe(`${ACTION_NAMES.FIX_LOGIC} in New Task`)
			expect(actions[4].title).toBe(`${ACTION_NAMES.FIX_LOGIC} in Current Task`)
			expect(actions[5].title).toBe(`${ACTION_NAMES.IMPROVE} in New Task`)
			expect(actions[6].title).toBe(`${ACTION_NAMES.IMPROVE} in Current Task`)
		})
