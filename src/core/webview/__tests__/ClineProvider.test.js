...
jest.requireMock("../../prompts/sections/custom-instructions").addCustomInstructions = mockAddCustomInstructions
// Mock delay module
jest.mock("delay", () => {
...
jest.mock("../../../integrations/misc/extract-text", () => ({
	extractTextFromFile: jest.fn().mockImplementation(async (_filePath) => {
...
describe("ClineProvider", () => {
	...
	beforeEach(() => {
		...
		mockContext = {
			...
			extensionUri: {},
			globalState: {
				get: jest.fn().mockImplementation((key) => globalState[key]),
				update: jest.fn().mockImplementation((key, value) => (globalState[key] = value)),
				keys: jest.fn().mockImplementation(() => Object.keys(globalState)),
			},
			secrets: {
				get: jest.fn().mockImplementation((key) => secrets[key]),
				store: jest.fn().mockImplementation((key, value) => (secrets[key] = value)),
				delete: jest.fn().mockImplementation((key) => delete secrets[key]),
			},
			subscriptions: [],
			extension: {
	...
	describe("deleteMessage", () => {
		...
		test('handles "Just this message" deletion correctly', async () => {
			...
			vscode.window.showInformationMessage.mockResolvedValue("confirmation.just_this_message")
			// Setup mock messages
			const mockMessages = [
		...
		test('handles "This and all subsequent messages" deletion correctly', async () => {
			...
			vscode.window.showInformationMessage.mockResolvedValue("confirmation.this_and_subsequent")
			// Setup mock messages
			const mockMessages = [
