...
export class ClineProvider extends EventEmitter {
	...
	settingsImportedAt
	latestAnnouncementId = "apr-23-2025-3-14" // Update for v3.14.0 announcement
    ...
    */
	...
	async initClineWithSubTask(parent, task, images) {
	...
	// task is finished.
	async initClineWithTask(task, images, parentTask, options = {}) {
		const {
			apiConfiguration,
			customModePrompts,
	...
	setWebviewMessageListener(webview) {
		const onReceiveMessage = async (message) => webviewMessageHandler(this, message)
	...
	async updateApiConfiguration(providerSettings) {
	...
	// Save configuration
	async upsertApiConfiguration(configName, apiConfiguration) {
		try {
