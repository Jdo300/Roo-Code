...
export async function initializeWorkingParser() {
	const TreeSitter = jest.requireActual("web-tree-sitter")
	...
	TreeSitter.Language.load = async (wasmPath) => {
...
export async function testParseSourceCodeDefinitions(testFilePath, content, options = {}) {
	// Set default options
...
export async function inspectTreeStructure(content, language = "typescript") {
	const TreeSitter = await initializeTreeSitter()
	const parser = new TreeSitter()
